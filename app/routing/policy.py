...
class RoutingPolicy(BaseModel):
    ...
    complexity_weight: float = Field(default=3.0, ge=0.0)

    complexity_reasoning_threshold: int = Field(default=4, ge=1, le=5)

    quant_efficiency_weight: float = Field(default=2.0, ge=0.0)

    language_match_weight: float = Field(default=4.0, ge=0.0)

    battery_efficiency_weight: float = Field(default=2.0, ge=0.0)
