...
def test_complete_includes_ttl_in_payload_when_given(provider, monkeypatch):
    monkeypatch.setattr(provider, "_ensure_ready", AsyncMock())
    ...
    asyncio.run(
        provider.complete(
            "model-a", [{"role": "user", "content": "hi"}], ttl_seconds=300
        )
    )

    assert captured["payload"]["ttl"] == 300


def test_complete_omits_ttl_from_payload_when_not_given(provider, monkeypatch):
    monkeypatch.setattr(provider, "_ensure_ready", AsyncMock())
    ...
    assert "ttl" not in captured["payload"]
